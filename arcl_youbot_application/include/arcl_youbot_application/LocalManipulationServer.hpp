...
//arc_youbot_planner
#include <arcl_youbot_application/LocalManipulationAction.h>
...
class LocalManipulationServer
{
public:
    LocalManipulationServer(std::string name);
    ~LocalManipulationServer(void);
    void goalCB();
    ...
    void FromGraspPoseToBasePose(double target_x, double target_y, double target_z, double primary_axis_x, double primary_axis_y, double cylin_r, double q1, double q5, double theta, geometry_msgs::Pose &base_pose);
    void GenerateSamplesForArmConf(double target_x, double target_y, double target_z, 
    double &min_cylin_r, double &max_cylin_r, double &cylin_z, std::vector<double>& q1_list, std::vector<double>& q5_list, double& theta);
protected:
    
    ros::NodeHandle nh_;
    // ManipulationAction 
    actionlib::SimpleActionServer<arcl_youbot_application::LocalManipulationAction> as_;
    std::string action_name_;
    arcl_youbot_application::LocalManipulationFeedback feedback_;
    arcl_youbot_application::LocalManipulationResult result_;
    arcl_youbot_application::LocalManipulationGoal goal_;
    ...
    double target_y_;
    double target_z_;
    double primary_axis_x_;
    double primary_axis_y_;
