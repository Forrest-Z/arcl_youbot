#include "arcl_youbot_application/LocalManipulationServer.hpp"

LocalManipulationServer::LocalManipulationServer(std::string name) : 
    as_(nh_, name, false),
    action_name_(name)
  {
    //register the goal and feeback callbacks
    as_.registerGoalCallback(boost::bind(&LocalManipulationServer::goalCB, this));
    as_.registerPreemptCallback(boost::bind(&LocalManipulationServer::preemptCB, this));
...
LocalManipulationServer::~LocalManipulationServer(void)
{
}

void LocalManipulationServer::ManipulationSceneToGraspScene(arcl_youbot_application::PlanningSceneMsg m_scene, youbot_grasp::PlanningScene& g_scene)
{
...
void LocalManipulationServer::FromGraspPoseToBasePose(double target_x, double target_y, double target_z, double primary_axis_x, double primary_axis_y, double cylin_r, double q1, double q5, double theta, geometry_msgs::Pose &base_pose)
{
    ...
    // }else{ 
        double y_axis_x_prime = primary_axis_x*cos(q5) - primary_axis_y*sin(q5);
        double y_axis_y_prime = primary_axis_x*sin(q5) + primary_axis_y*cos(q5);
        y_axis_y_prime = -y_axis_y_prime;
        y_axis_x_prime = -y_axis_x_prime;

        joint_1_x = target_x +/* x_axis.x()*0.005*/ + cylin_r*y_axis_x_prime;
        joint_1_y = target_y +/* x_axis.y()*0.005*/ + cylin_r*y_axis_y_prime;        
...
// currently works with up down grasp
void LocalManipulationServer::GenerateSamplesForArmConf(double target_x, double target_y, double target_z, 
    double &min_cylin_r, double &max_cylin_r, double &cylin_z, std::vector<double>& q1_list, std::vector<double>& q5_list, double& theta)
{
   ...
   // 0.067 is the length of the gripper finger
    ...
    min_cylin_r = 0.24*sin(theta) + 0.033 + 0.21;  // 0.2 this is the pick distance 
...
void LocalManipulationServer::goalCB()
{
    ...
    target_x_ = new_goal_->object_x;
    target_y_ = new_goal_->object_y;
    target_z_ = new_goal_->object_z;
    primary_axis_x_ = new_goal_->primary_axis_x;
    primary_axis_y_ = new_goal_->primary_axis_y;
    ...
    GenerateSamplesForArmConf(target_x_, target_y_, target_z_, min_cylin_r, max_cylin_r, cylin_z, q1_list, q5_list, theta);
    ...
    for(cylin_r = min_cylin_r; cylin_r < max_cylin_r; cylin_r += 0.005){
        ...
        for(int j = 0; j < q1_list.size(); j++){
            ...
            // if(theta - M_PI < 0.1 && theta - M_PI > -0.1){
                ...
                for(int k = 0; k < q5_list.size(); k ++){
                    ...
                    FromGraspPoseToBasePose(target_x_, target_y_, target_z_, primary_axis_x_, primary_axis_y_, cylin_r, q1, q5, theta, base_pose);
        ...
        while(final_base_pose_list.size() > 0){ 
            ...
            for(int p = 0; p < final_base_pose_list.size(); p++){
                temp_dist = sqrt((final_base_pose_list[p].position.x)*(final_base_pose_list[p].position.x) + (final_base_pose_list[p].position.y)*(final_base_pose_list[p].position.y));
...
bool LocalManipulationServer::planInAdvance(geometry_msgs::Pose final_base_pose, std::vector<double> test_group_variable_values, ros::NodeHandle& node_handle){
    return true;
}


void LocalManipulationServer::preemptCB()
{
...
{
    ros::init(argc, argv, "localmanipulation_action");

    LocalManipulationServer manipulation_planner_0("localmanipulation_action");
    //LocalManipulationServer manipulation_planner_1("manipulation_action_1");    
    ros::spin();
