...
geometry_msgs::Pose PlacePlanAction::getRetractPose(geometry_msgs::Pose target_pose, double x_length, double y_length, double z_length){
  ...
  while(step_index < max_step){ 
    ...
    retract_pose.position.x += combined_vector_x*0.015;
    retract_pose.position.y += combined_vector_y*0.015;
...
#endif
#ifdef USE_SERVICE
bool PlacePlanAction::goalCB(placeplan_service::PlacePlan::Request  &req,
          placeplan_service::PlacePlan::Response &res)
#endif
{ 
  #ifdef USE_ACTIONLIB
    ...
    std::cout << "Position: " << req.target_object_pose.position.x << ", " << 
                                 req.target_object_pose.position.y << ", " << 
                                 req.target_object_pose.position.z << ", " << std::endl;
    std::cout << "Dimensions: " << req.x_dim << ", " << 
                                   req.y_dim << ", " << 
                                   req.z_dim << ", " << std::endl;
    std::cout << "Call received!" << std::endl;
...
void PlacePlanAction::initWorldPointCloudFromFile(std::string pc_filename){
