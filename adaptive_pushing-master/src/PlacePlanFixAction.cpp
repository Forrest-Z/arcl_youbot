...
class PlacePlanFixAction
{
public:
    PlacePlanFixAction(std::string name) : as_(nh_, name, false),
                                                action_name_(name)
    {
        ROS_INFO_STREAM("start place plan server!");
//register the goal and feeback callbacks
#ifdef USE_ACTIONLIB
        as_.registerGoalCallback(boost::bind(&PlacePlanFixAction::goalCB, this));
        as_.start();
#endif
...
void PlacePlanFixAction::convert3dUnOrganized(cv::Mat &objDepth, Eigen::Matrix3f &camIntrinsic, pcl::PointCloud<pcl::PointXYZ>::Ptr objCloud)
{
...
void PlacePlanFixAction::readDepthImage(cv::Mat &depthImg, std::string path)
{
...
void PlacePlanFixAction::toTransformationMatrix(Eigen::Matrix4f &camPose, std::vector<double> camPose7D)
{
...
void PlacePlanFixAction::InverseTransformationMatrix(Eigen::Matrix4f &origin, Eigen::Matrix4f &inverse)
{
...
Eigen::Matrix4f PlacePlanFixAction::transformPclFromWorldToTarget(pcl::PointCloud<pcl::PointXYZ>::Ptr target_pcl, geometry_msgs::Pose target_pose)
{
...
bool PlacePlanFixAction::checkRegionCollision(geometry_msgs::Pose center, double x_length, double y_length, double z_length)
{
...
geometry_msgs::Pose PlacePlanFixAction::getRetractPose(geometry_msgs::Pose target_pose, double x_length, double y_length, double z_length)
{
    int max_step = 25;
    int step_index = 0;
...
#ifdef USE_ACTIONLIB
void PlacePlanFixAction::goalCB()
#endif
#ifdef USE_SERVICE
    bool PlacePlanFixAction::goalCB(placeplan_service::PlacePlan::Request &req,
                                         placeplan_service::PlacePlan::Response &res)
#endif
{
#ifdef USE_ACTIONLIB
    ...
    geometry_msgs::Pose pre_push_pose = target_object_pose_;
    pre_push_pose.position.x += 0.04;
    pre_push_pose.position.y -= 0.04;
    publish_cube(pre_push_pose, x_length, y_length, z_length, retract_cube_pub, 1, 0, 0);
...
void PlacePlanFixAction::initWorldPointCloudFromFile(std::string pc_filename)
{
...
void PlacePlanFixAction::initWorldPointCloud()
{
...
void PlacePlanFixAction::publish_cube(geometry_msgs::Pose pose, double x, double y, double z, ros::Publisher pub, double r, double g, double b)
{
...
void PlacePlanFixAction::publish_arrow(geometry_msgs::Point start, geometry_msgs::Point end)
{
    std::cout << "Publishing marker..." << std::endl;
...
{
    ros::init(argc, argv, "placeplan_action_fix");
    ROS_WARN_STREAM("starting");
    ros::NodeHandle nh;
    PlacePlanFixAction placeplan_action(ros::this_node::getName());
#ifdef USE_SERVICE
    ros::ServiceServer service = nh.advertiseService("/placeplan_service_fix", &PlacePlanFixAction::goalCB, &placeplan_action);
#endif
