    ...
    node_(&node)
{
    ...
    node_->getParamNames(param_keys);
    if(param_keys.size() > 0){ 
        for(int i = 0; i < param_keys.size(); i++){
            ROS_WARN(param_keys[i].c_str());
        }
    }else{
        ROS_WARN("no param_keys");
    }
    ROS_WARN_STREAM("namespace:"<<node_->getNamespace());
    std::string use_gazebo;
    node_->getParam("luh_youbot_controller/use_gazebo_simulation", use_gazebo);
    ROS_WARN_STREAM("use_gazebo:"<<use_gazebo.c_str());
...
//########################## CALLBACK: GET POSE ########################################################################
bool ControllerNode::getPoseCallback(arcl_youbot_msgs::GetArmPose::Request &req,
                                  arcl_youbot_msgs::GetArmPose::Response &res)
{
