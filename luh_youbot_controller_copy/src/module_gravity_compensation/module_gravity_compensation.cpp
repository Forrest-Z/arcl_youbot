...
bool ModuleGravityCompensation::deactivateCallback(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
    ...
    do_compensation_ = false;
    //youbot_->arm()->setJointVelocities(JointVelocity());
    arcl_youbot_kinematics::JointPosition position_now;
...
//########## DEACTIVATE ################################################################################################
bool ModuleGravityCompensation::getLoadCallback(arcl_youbot_msgs::GetLoad::Request &req,
                                                arcl_youbot_msgs::GetLoad::Response &res)
{    
    boost::mutex::scoped_lock lock(arm_mutex_);
...
void ModuleGravityCompensation::forceFitCallback()
{
    ...
    boost::shared_ptr<const arcl_youbot_msgs::ForceFitGoal> goal = force_fit_server_->acceptNewGoal();
...
//########## CHECK GOAL ################################################################################################
bool ModuleGravityCompensation::checkGoal(const arcl_youbot_kinematics::JointPosition &pos)
{
