...
private:
  ...
  // only for use for object type 'cube' to accommodate different lengths of cubes 
  double dx_;
  double dy_;
  double dz_;
 ...
 */                  
  ...
  void set_target_dimension(double dx, double dy, double dz){dx_ = dx; dy_ = dy;dz_ = dz;}
};
