...
bool GraspPlanner::plan(std::string object_file_name, std::list<plannedGrasp*> & grasp_list)
{
  ...
  for(auto p = sampledPoint_list.begin(); p != sampledPoint_list.end(); p++){
    delete *p;
  }
  sampledPoint_list.clear();
...
{
	approach_dir_generator_->generateApproachDir(object_file_name, target_pose_, dx_, dy_, dz_,sampledPoint_list, &sample_point, &sample_normal);
  ROS_WARN_STREAM("grasp point number:"<<sampledPoint_list.size());
}
...
void GraspPlanner::getGrasp(std::string object_file_name)
{
  ...
  grasp_generator_->generateGrasp(object_file_name, sample_point, sample_normal, dx_, dy_, dz_, planned_grasps_, gripper_type_, planning_scene_, target_pose_, sampledPoint_list); 	
}
