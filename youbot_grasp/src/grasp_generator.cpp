...
/*constructor*/
  std::vector<fcl::Vec3f> GraspGenerator::vertices_L_;
  std::vector<fcl::Triangle> GraspGenerator::triangles_L_;
  std::vector<fcl::Vec3f> GraspGenerator::vertices_T_;
  std::vector<fcl::Triangle> GraspGenerator::triangles_T_;
  std::vector<fcl::Vec3f> GraspGenerator::vertices_cube_;
  std::vector<fcl::Triangle> GraspGenerator::triangles_cube_;

  std::vector<fcl::Vec3f> GraspGenerator::vertices_cube_1_;
  std::vector<fcl::Triangle> GraspGenerator::triangles_cube_1_;
  std::vector<fcl::Vec3f> GraspGenerator::vertices_cube_2_;
  std::vector<fcl::Triangle> GraspGenerator::triangles_cube_2_; 
...
GraspGenerator::GraspGenerator(float R, ros::NodeHandle &node)
{
  radius = R;    
  node_ = &node;
loadOBJFile((ros::package::getPath("youbot_grasp")+"/models/T_block.obj").c_str(), GraspGenerator::vertices_T_, GraspGenerator::triangles_T_);
loadOBJFile((ros::package::getPath("youbot_grasp")+"/models/L_block.obj").c_str(), GraspGenerator::vertices_L_, GraspGenerator::triangles_L_);
  ...
  loadOBJFile((ros::package::getPath("youbot_grasp")+"/models/cube_1.obj").c_str(), GraspGenerator::vertices_cube_1_, GraspGenerator::triangles_cube_1_);
  loadOBJFile((ros::package::getPath("youbot_grasp")+"/models/cube_2.obj").c_str(), GraspGenerator::vertices_cube_2_, GraspGenerator::triangles_cube_2_);
...
*/
int GraspGenerator::generateGrasp(std::string object_file_name, std::vector<geometry_msgs::Point>& sample_point_not , std::vector<geometry_msgs::Point>& sample_normal_not, double dx, double dy, double dz, std::list<plannedGrasp*>& grasp_list, int gripper_type, youbot_grasp::PlanningScene * planning_scene, geometry_msgs::Pose target_pose, std::list<sampledPoint*> sampledPoint_list)
{
  MyMesh m, res;                
  ...
  if(gripper_type == 2){
    ...
    for(int i = 0;i < sample_point.size();++i)
    {
      ...
      for(int j = 0;j < 1;++j)  // every 30 degrees rotation of the hand
      {
        ...
        if(object_file_name == "cube" /*|| object_file_name == "cube_1" || object_file_name == "cube_2"*/){ 
        ROS_WARN_STREAM("this object is cube");
        palm_obj_m = this->get_cp_obj_quaternion(sample_point[i], sample_normal[i], M_PI/2 /*M_PI/24 *j*/);  
        }else{ 
        ROS_WARN_STREAM("this object is L or T");
        palm_obj_m = this->get_cp_obj_quaternion(sample_point[i], sample_normal[i], 0/*M_PI/24 *j*/);  
        ...
        //  vrep_gripper_pose.position.z -= 0.0285;
          ...
          for(int p = 0;p < planning_scene->scene_object_pose.size();p++){
            ...
            //ROS_WARN_STREAM("sceneobject pose:"<<planning_scene->scene_object_pose[p].position.x<<","<<planning_scene->scene_object_pose[p].position.y<<","<<planning_scene->scene_object_pose[p].position.z);
          }
          // while( getchar() != '\n' );
          //check collision between gripper with target object
          // to detect different size of cubes, just for temporary use
          //if(std::abs(dz - 0.15)<0.01){
          //   object_file_name = "cube";
          //}else if(std::abs(dz - 0.7) < 0.01){
          //   object_file_name = "cube_1";
          //}else if(std::abs(dz - 0.988) < 0.01){
          //   object_file_name = "cube_2";
          //}
...
bool GraspGenerator::checkCollision(std::string object_1, std::string object_2,fcl::Vec3f T1, fcl::Vec3f T2, fcl::Matrix3f R1, fcl::Matrix3f R2){
  ...
  }else if(object_1 == "cube_1"){
    vertices1 = vertices_cube_1_;
    triangles1 = triangles_cube_1_;
  }else if(object_1 == "cube_2"){
    vertices1 = vertices_cube_2_;
    triangles1 = triangles_cube_2_;
  }else if(object_1 == "L_block"){
    vertices1 = vertices_L_;
    triangles1 = triangles_L_;
  }else if(object_1 == "T_block"){
    vertices1 = vertices_T_;
    triangles1 = triangles_T_;
  ...
  }else if(object_2 == "cube_1"){
    vertices2 = vertices_cube_1_;
    triangles2 = triangles_cube_1_;
  }else if(object_2 == "cube_2"){
    vertices2 = vertices_cube_2_;
    triangles2 = triangles_cube_2_;
  }else if(object_2 == "L_block"){
    vertices2 = vertices_L_;
    triangles2 = triangles_L_;
  }else if(object_2 == "T_block"){
    vertices2 = vertices_T_;
    triangles2 = triangles_T_;
  ...
  fcl::CollisionObject* obj1 = new fcl::CollisionObject(std::shared_ptr<fcl::BVHModel<fcl::OBBRSS>>(m1), pose1);
  fcl::CollisionObject* obj2 = new fcl::CollisionObject(std::shared_ptr<fcl::BVHModel<fcl::OBBRSS>>(m2), pose2);
...
tf::Matrix3x3 GraspGenerator::get_cp_obj_quaternion(geometry_msgs::Point grasp_point, geometry_msgs::Point grasp_dir, double rotate_around_x)
{
  ...
  ROS_WARN_STREAM("get_cp_obj_quaternion: a:"<<a<<",b:"<<b<<",c:"<<c);
  // ROS_INFO_STREAM(a);
  // ROS_INFO_STREAM(b);
  // ROS_INFO_STREAM(c);
  ...
  //ROS_WARN_STREAM("new_ro_z_axis:"<<new_ro_z_axis.x<<","<<new_ro_z_axis.y<<","<<new_ro_z_axis.z);
  //ROS_WARN_STREAM("new_ro_x_axis:"<<new_ro_x_axis.x<<","<<new_ro_x_axis.y<<","<<new_ro_x_axis.z);
  //ROS_WARN_STREAM("new_ro_y_axis:"<<new_ro_y_axis.x<<","<<new_ro_y_axis.y<<","<<new_ro_y_axis.z);
