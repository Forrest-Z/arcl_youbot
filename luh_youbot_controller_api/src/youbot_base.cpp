...
void YoubotBase::init(ros::NodeHandle &node, bool async)
{
    ...
    // service client
    get_pose_client_ = node.serviceClient<arcl_youbot_msgs::GetBasePose>("youbot_base/get_pose");
...
{
    arcl_youbot_msgs::MoveBaseGoal goal;
    goal.x = x;
    goal.y = y;
    goal.theta = theta;
...
{
    arcl_youbot_msgs::ApproachBaseGoal goal;
    if(right < 0)
    {
        goal.left = -right;
        goal.right = 0;
...
{
    arcl_youbot_msgs::GetBasePose srv;
