...
def filter_contours_by_area(contours, min_area = 0, max_area = np.inf):
    if len(contours) == 0:
        return np.array([])
...
def find_object_contours(image, hsv_lower_value,
                         hsv_upper_value, kernel):
...
def calculate_distance_mtx(tracked_objects, points):
...
def remove_object_if_too_many_frames_skipped(tracked_objects, assignment,
                                             max_skipped_frames):
    for i, tracked_obj in enumerate(tracked_objects):
